...
interface TransformForm {
  id: number;
  transformedDataframeName: string;
  sqlText: string;
}

const TransformTabContent: React.FC = () => {
  const [transformForms, setTransformForms] = useState<TransformForm[]>([
    {
      id: 1,
      transformedDataframeName: '',
      sqlText: ''
    }
  ]);
  ...
  return (
    <CForm>
      <div className="d-flex justify-content-between align-items-center mb-2">
        ...
        <CButton color="success" size="sm" onClick={addTransformForm}>+ Add Transform</CButton>
      </div>
        ...
        <CCard className="mb-3" key={`transform-${form.id}`}>
          <CCardBody>
            ...
            <CRow className="mb-3">
              ...
              <CCol md={10}>
                <CFormLabel>Transformed Dataframe Name</CFormLabel>
                <CFormInput type="text" value={form.transformedDataframeName} onChange={(e) => handleInputChange(form.id, 'transformedDataframeName', e.target.value)} placeholder="Enter Transformed Dataframe Name" />
              </CCol>
            </CRow>
...
export default TransformTabContent;
